                ...
                if (oItem) {
                    ...
                    if (oBindingContext) {
                        try {
                            ...
                            console.error("Invoice 삭제 중 오류 발생:", error);
                            let errorMessage = "Invoice 삭제에 실패했습니다.";
                            if (error.response) {
                                console.error("서버 응답:", error.response);
                                errorMessage += " 서버 응답: " + (error.response.statusText || error.response.status);
                                console.error("응답 데이터:", error.response.data);
                            }
                            if (error.request) {
                                console.error("요청 정보:", error.request);
                                console.error("요청 URL:", error.request.url);
                                console.error("요청 메소드:", error.request.method);
                            }
                            if (error.message) {
                            ...
                            // OData 모델의 에러 정보 확인
                            const oModel = this.getView().getModel("invoice");
                            if (oModel && oModel.getMessagesByPath) {
                                const aMessages = oModel.getMessagesByPath(sSelectedPath);
                                console.error("OData 모델 메시지:", aMessages);
                            }
                        }
                } else {
                    console.error("선택된 항목을 찾을 수 없습니다.");
                    MessageToast.show("선택된 항목이 더 이상 존재하지 않습니다.");
                }
            } else {
